...
def login_admin(
    ...
    token = login(db, str(payload.email), payload.password)
...
def session(
    ...
    session_data = get_session(db, current_admin.email)
    return success_response(session_data, message="Session refreshed")
